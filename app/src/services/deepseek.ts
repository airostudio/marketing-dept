 ...
 * Provides cost-effective content generation using DeepSeek's API
...
class DeepSeekService {
  ...
  private defaultModel: DeepSeekConfig['model'] = 'deepseek-chat'
...
Focus on clarity, engagement, and achieving marketing objectives. ${request.context ? `Context: ${request.context}` : ''}`
  ...
  private calculateCost(tokens: number, model: string): number {
    const pricingPer1M = {
      'deepseek-chat': 0.14, // Cache miss input price
      'deepseek-reasoner': 0.55,
    }

    const price = pricingPer1M[model as keyof typeof pricingPer1M] || 0.14
